...
COLLECTION_POLICY_RESPONSE_MCP_ACCESS_BINDINGS = "response_filter_mcp_access_bindings"
...
def _filter_payload_mcp_access_bindings(
    payload: dict[str, object],
    authorizer: "KubernetesAuthorizer",
    identity: "_RequestIdentity",
    workspace_name: str,
) -> bool:
    bindings = payload.get("mcp_access_bindings")
    if bindings is None:
        return True
    if not isinstance(bindings, list):
        return False
    payload["mcp_access_bindings"] = [
        binding
        for binding in bindings
        if isinstance(binding, dict)
        and (server_name := _normalize_string(binding.get("server_name")))
        and _is_allowed_named_resource(
            authorizer,
            identity,
            workspace_name,
...
def apply_response_collection_filters(
    ...
    *,
    ...
    for policy in applied_policies:
        ...
        elif policy == COLLECTION_POLICY_RESPONSE_MCP_ACCESS_BINDINGS:
            enforceable &= _filter_payload_mcp_access_bindings(
                filtered_payload, authorizer, identity, workspace_name
            )
